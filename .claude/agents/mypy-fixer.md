...
Fixes mypy type errors and eliminates `Any` usage in the smlab project.
...
1. Fix all mypy errors reported by `uv run mypy smlab`
...
Place new TypedDict classes in `smlab/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`smlab/typing.py` when shared.
...
1. Run `uv run mypy smlab` and capture all errors.
...
4. Run `uv run mypy smlab` again to verify fixes.
