---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the smlab project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the smlab project.
...
- `` :py:mod:`smlab.string` `` for modules
- `` :py:func:`smlab.string.slugify` `` for functions
- `` :py:class:`smlab.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~smlab.string.slugify` `` renders as `slugify`
