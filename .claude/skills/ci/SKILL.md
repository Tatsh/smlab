...
If any changed files are under `smlab/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `smlab/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `smlab/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `smlab/commands/main.py`,
...
For Python files, strip the `smlab/` prefix and replace `/` with `.` (like module imports).

- Python file `smlab/media.py` → `media:`.
- Multiple files under `smlab/commands/` → `commands:`.
- Single command file `smlab/commands/admin.py` → `commands.admin:`.
