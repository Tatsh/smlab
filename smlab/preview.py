...
``#SAMPLELENGTH`` needs no model: 83% of the corpus uses exactly fifteen seconds and a further 6%
uses twelve, so it is a convention rather than a per-song judgement.

``#SAMPLESTART`` is a real decision, and the corpus supplies 3,719 human examples of it. The task is
framed as choosing one measure out of the whole song, so the model emits a score per measure and a
softmax over the song picks the winner. That framing suits a problem with exactly one label per
example and keeps the answer on a musical boundary, which is where charters put it about thirteen
...
Eighty-three per cent of the corpus uses this exact value, so it is a constant rather than something
to predict.
...
Clamping to the span charters actually use trims the model's worst mistakes: on held-out songs it
moves the ninetieth percentile error from 63 to 56 seconds without costing central accuracy.
...
def measure_features(features: NDArray[np.float16]) -> NDArray[np.float32]:
    ...
    A relative position channel is appended because charters favour the middle of a song, and the
    model should be able to use that prior alongside the audio evidence rather than having to infer
    it from length alone.
...
class PreviewModel(nn.Module):
    ...
    def forward(self, features: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        :py:class:`~torch.Tensor`
            Logits shaped ``(batch, measures)``, with padding set to negative infinity so that a
            softmax ignores it.
        """
...
def predict_sample_start(
    ...
    # Clamp in measure space rather than seconds, so the guardrail cannot land the preview part-way
    # through a bar.
